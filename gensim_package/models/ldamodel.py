...
"""
**For a faster implementation of LDA (parallelized for multicore machines), see** :mod:`gensim_package.models.ldamulticore`.
...
logger = logging.getLogger('gensim_package.models.ldamodel')
...
class LdaModel(interfaces.TransformationABC, basemodel.BaseTopicModel):
    ...
    def __init__(self, corpus=None, num_topics=100, id2word=None,
        ...
        Turn on `distributed` to force distributed computing (see the `web tutorial <http://radimrehurek.com/gensim_package/distributed.html>`_
        on how to set up a cluster of machines for gensim_package).
    ...
    def update(self, corpus, chunksize=None, decay=None, offset=None,
        ...
        Args:
            corpus (gensim_package corpus): The corpus with which the LDA model should be updated.
    ...
    def save(self, fname, ignore=['state', 'dispatcher'], *args, **kwargs):
        ...
        keep in mind:
          ...
          2. The `save` method does not automatically save all NumPy arrays using NumPy, only
             those ones that exceed `sep_limit` set in `gensim_package.utils.SaveLoad.save`. The main
             concern here is the `alpha` array if for instance using `alpha='auto'`.

        Please refer to the wiki recipes section (https://github.com/piskvorky/gensim_package/wiki/Recipes-&-FAQ#q9-how-do-i-load-a-model-in-python-3-that-was-trained-and-saved-using-python-2)
