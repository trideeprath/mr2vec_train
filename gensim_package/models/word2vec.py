...
For a blog tutorial on gensim_package word2vec, with an interactive web app trained on GoogleNews, visit http://radimrehurek.com/2014/02/word2vec-tutorial/

**Make sure you have a C compiler before installing gensim_package, to use optimized (compiled) word2vec training**
...
Note that there is a :mod:`gensim_package.models.phrases` module which lets you automatically
detect phrases longer than one word. Using phrases, you can learn a word2vec model
where "words" are actually multiword expressions, such as `new_york_times` or `financial_crisis`:

>>> bigram_transformer = gensim_package.models.Phrases(sentences)
...
.. [3] Optimizing word2vec in gensim_package, http://radimrehurek.com/2013/09/word2vec-in-python-part-two-optimizing/
"""
...
try:
    ...
    import abcedfdf
    #from gensim_package.models.word2vec_inner import train_batch_sg, train_batch_cbow
    #from gensim_package.models.word2vec_inner import score_sentence_sg, score_sentence_cbow
    #from gensim_package.models.word2vec_inner import FAST_VERSION, MAX_WORDS_IN_BATCH

except ImportError:
    print("TD import errror")
    ...
    def train_batch_sg(model, sentences, alpha, work=None):
        ...
        This is the non-optimized, Python version. If you have cython installed, gensim_package
        will use the optimized version from word2vec_inner instead.

        """
        result = 0
        for sentence in sentences:
            # Filter words having high frequency
    ...
    def train_batch_cbow(model, sentences, alpha, work=None, neu1=None):
        ...
        result = 0
        for sentence in sentences:
            # Action item : Remove the action item
            word_vocabs = [model.vocab[w] for w in sentence if w in model.vocab and
                           model.vocab[w].sample_int > model.random.rand() * 2**32]
            for pos, word in enumerate(word_vocabs):
                reduced_window = model.random.randint(model.window)  # `b` in the original word2vec code
                ...
                # word : word to predict , word2_indices : input words, l1 : mean of the vectors of the input vectors, alpha : learning rate
    ...
    def train_batch_cbow_rel(model, sentences, alpha, work=None, neu1=None):
        """Update CBOW model by training on a sequence of sentences.
        Each sentence is a list of string tokens, which are looked up in the model's
        vocab dictionary. Called internally from `Word2Vec.train()`.
        This is the non-optimized, Python version. If you have cython installed, gensim_package
        ...
        for sentence in sentences:
            ...
            if len(word_vocabs) != 3:
                continue
            word = word_vocabs[2]
            word2_indices = [word_vocabs[0].index, word_vocabs[1].index]
...
def train_cbow_pair_rel(model, word, input_word_indices, l1, alpha, learn_vectors=True, learn_hidden=True):
    neu1e = zeros(l1.shape)
    ...
    if model.negative:
        # use this word (label = 1) + `negative` other random words not from this sentence (label = 0)
        word_indices = [word.index]
        while len(word_indices) < model.negative_rel + 1:
            w = model.cum_table.searchsorted(model.random.randint(model.cum_table[-1]))
            if w != word.index:
                word_indices.append(w)
        l2b = model.syn1neg[word_indices]  # 2d matrix, k+1 x layer1_size
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            ...
            sg=0, hs=0, negative=5, negative_rel=5, cbow_mean=1, hashfxn=hash, iter=5, null_word=0,
    ...
    def build_vocab(self, sentences, keep_raw_vocab=False, trim_rule=None, progress_per=10000, update=False):
        ...
        print("Scan Vocab done")
        self.scale_vocab(keep_raw_vocab=keep_raw_vocab, trim_rule=trim_rule, update=update)  # trim by min_count & precalculate downsampling
        print("Scale Vocab done")
        self.finalize_vocab(update=update)  # build tables & arrays
        print("Finalize Vocab done")

    def scan_vocab(self, sentences, progress_per=10000, trim_rule=None, update=False):
        """Do an initial scan of all words appearing in sentences. Creates raw_vocab and corpus_count"""
    ...
    def scale_vocab(self, min_count=None, sample=None, dry_run=False, keep_raw_vocab=False, trim_rule=None, update=False):
        ...
        Creates vocab object dictionary and index2word dictionary.
        """
        ...
        for w in retain_words:
            v = self.raw_vocab[w] # My action item : check if word is a relation, dont downsample , just continue
    ...
    def _do_train_job_new(self, sentences, alpha, inits):
        work, neu1 = inits
        tally = 0
        word_sentences = []
        relation_sentences = []
    ...
    def train(self, sentences, total_words=None, word_count=0,
        ...
        def job_producer():
            ...
            if job_no == 0 and self.train_count == 0:
                logger.warning("train() called with an empty iterator (if not intended,iteration = an iterable).")

            # give the workers heads up that they can finish. Adds workers number of None jobs for all of them to quit
            for _ in xrange(self.workers):
                job_queue.put(None)
            print("job loop exiting, total %i jobs" % (job_no))

        # buffer ahead only a limited number of jobs.. this is the reason we can't simply use ThreadPool :(
        job_queue = Queue(maxsize=queue_factor * self.workers)
        progress_queue = Queue(maxsize=(queue_factor + 1) * self.workers)

        workers = [threading.Thread(target=worker_loop, name='worker_loop') for _ in xrange(self.workers)]
        unfinished_worker_count = len(workers)
        workers.append(threading.Thread(target=job_producer, name='job_producer'))
        ...
        # Wait for all worker_threads to finish
        while unfinished_worker_count > 0:
            ...
            if elapsed >= next_report:
                if total_examples:
                    ...
                    print("PROGRESS: at %.2f%% examples, %.0f words/s, in_qsize %i, out_qsize %i" %(100.0 * example_count / total_examples, trained_word_count / elapsed, utils.qsize(job_queue), utils.qsize(progress_queue)))
                    ####
                    #self.w2v_model_accuracy()
                    ####
    ...
    def score(self, sentences, total_sentences=int(1e6), chunksize=100, queue_factor=2, report_delay=1):
        ...
        See the article by [taddy]_ and the gensim_package demo at [deepir]_ for examples of how to use such scores in document classification.
        ...
        .. [deepir] https://github.com/piskvorky/gensim_package/blob/develop/docs/notebooks/deepir.ipynb

        """
        if FAST_VERSION < 0:
            import warnings
            warnings.warn("C extension compilation failed, scoring will be slow. "
                          "Install a C compiler and reinstall gensim_package for fastness.")
...
class BrownRelCorpus(object):
    ...
    def __iter__(self):
        for fname in os.listdir(self.dirname):
            fname = "gensim_package/corpus/brown_sen_rel.txt"
            if not os.path.isfile(fname):
                continue
            for line in utils.smart_open(fname):
                line = utils.to_unicode(line)
