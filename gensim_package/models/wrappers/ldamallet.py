...
The wrapped model can NOT be updated with new documents for online training -- use gensim_package's `LdaModel` for that.

Example:

>>> model = gensim_package.models.wrappers.LdaMallet('/Users/kofola/mallet-2.0.7/bin/mallet', corpus=my_corpus, num_topics=20, id2word=dictionary)
...
class LdaMallet(utils.SaveLoad):
    ...
    def __init__(self, mallet_path, corpus=None, num_topics=100, alpha=50, id2word=None, workers=4, prefix=None,
        ...
        `corpus` is a gensim_package corpus, aka a stream of sparse document vectors.
    ...
    def read_doctopics(self, fname, eps=1e-6, renorm=True):
        """
        Yield document topic vectors from MALLET's "doc-topics" format, as sparse gensim_package vectors.
...
def malletmodel2ldamodel(mallet_model, gamma_threshold=0.001, iterations=50):
    """
    Function to convert mallet model to gensim_package LdaModel. This works by copying the
    training model weights (alpha, beta...) from a trained mallet model into the
    gensim_package model.
    ...
    Returns:
    -------
    model_gensim : LdaModel instance; copied gensim_package LdaModel
    """
