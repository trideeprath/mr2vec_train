...
Example:
    ...
    >>> # train model
    >>> lda = gensim_package.models.wrappers.LdaVowpalWabbit('/usr/local/bin/vw',
                                                     corpus=corpus,
                                                     num_topics=20,
                                                     id2word=dictionary)
    ...
    >>> lda = gensim_package.models.wrappers.LdaVowpalWabbit.load('vw_lda.model')
...
def vwmodel2ldamodel(vw_model, iterations=50):
    """
    Function to convert vowpal wabbit model to gensim_package LdaModel. This works by
    simply copying the training model weights (alpha, beta...) from a trained
    vwmodel into the gensim_package model.
