...
def any2sparse(vec, eps=1e-9):
    """Convert a numpy/scipy vector into gensim_package document format (=list of 2-tuples)."""
...
def scipy2sparse(vec, eps=1e-9):
    """Convert a scipy.sparse vector into gensim_package document format (=list of 2-tuples)."""
    vec = vec.tocsr()
...
class Scipy2Corpus(object):
    """
    Convert a sequence of dense/sparse vectors into a streamed gensim_package corpus object.
...
class Dense2Corpus(object):
    """
    Treat dense numpy array as a sparse, streamed gensim_package corpus.
...
class Sparse2Corpus(object):
    """
    Convert a matrix in scipy.sparse format into a streaming gensim_package corpus.
...
def unitvec(vec, norm='l2'):
    ...
    Output will be in the same format as input (i.e., gensim_package vector=>gensim_package vector,
    ...
    if isinstance(first, (tuple, list)) and len(first) == 2: # gensim_package sparse format
        if norm == 'l1':
