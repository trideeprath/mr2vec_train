...
The resulting object can be used as input to all gensim_package models (TFIDF, LSI, ...),
serialized with any format (Matrix Market, SvmLight, Blei's LDA-C format etc).

See the `gensim_package.test.test_miislita.CorpusMiislita` class for a simple example.
"""
...
logger = logging.getLogger('gensim_package.corpora.textcorpus')
