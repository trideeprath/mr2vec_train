...
While the standard corpus interface in gensim_package allows iterating over corpus with
...
logger = logging.getLogger('gensim_package.corpora.indexedcorpus')
...
class IndexedCorpus(interfaces.CorpusABC):
    def __init__(self, fname, index_fname=None):
        ...
        >>> gensim_package.corpora.SvmLightCorpus.serialize('testfile.svmlight', corpus)
        >>> # load back as a document stream (*not* plain Python list)
        >>> corpus_with_random_access = gensim_package.corpora.SvmLightCorpus('tstfile.svmlight')
