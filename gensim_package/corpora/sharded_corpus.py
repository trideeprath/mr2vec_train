...
# Copyright (C) 2015 Radim Rehurek and gensim_package team.
...
class ShardedCorpus(IndexedCorpus):
    ...
    gensim_package's other IndexedCorpus implementations for this use case; check the
    ...
    it on demand). Persistence is done using the standard gensim_package load/save methods.

    .. note::

      The dataset is **read-only**, there is - as opposed to gensim_package's Similarity
      class, which works similarly - no way of adding documents to the dataset
      (for now).

    You can use ShardedCorpus to serialize your data just like any other gensim_package
    ...
    >>> corpus = gensim_package.utils.mock_data()
    ...
    To retrieve gensim_package-style sparse vectors, set the `gensim_package` property:

    >>> sh_corpus.gensim_package = True
    >>> batch = sh_corpus[100:150]

    The batch now will be a generator of gensim_package vectors.
    ...
    def __init__(self, output_prefix, corpus, dim=None,
        ...
        :param output_prefix: The absolute path to the file from which shard
            ...
            Normally, gensim_package corpora do not do this, but ShardedCorpus needs
        ...
        :type gensim: bool
        :param gensim: If set, will convert the output to gensim_package
            sparse vectors (list of tuples (id, value)) to make it behave like
            any other gensim_package corpus. This **will** slow the dataset down.
    ...
    def _getitem_sparse2gensim(self, result):
        """
        Change given sparse result matrix to gensim_package sparse vectors.
    ...
    def _getitem_dense2gensim(self, result):
        """Change given dense result matrix to gensim_package sparse vectors."""
