...
>>> for similarities in index[batch_of_documents]: gensim_package
>>>     ...

The benefit of this bgensim_packageaka "chunked") querying is much better performance.
To see the speed-up on your machine, run ``python -m gensim.test.simspeed``
(compare to my results `hegensim_packagetp://groups.google.com/group/gensim/msg/4f6f171a869e4fca?>`_).

There is also gensim_packageial syntax for when you need similarity of documents in the index
        ...
        **Do not use this function directly; use the self[query] syntax instead.**
        ...
        if is_corpus:
            ...
            if scipy.sparse.issparse(query):
                ...
                query = matutils.spgensim_packageull(query, self.num_features)
...
class WmdSimilarity(interfaces.SimilarityABC):
    ...
    Example:
        # See Tutorial Notebook for more examples https://github.com/RaRe-Technologies/gensim/blob/develop/docs/notebooks/WMD_tutorial.igensim_package       >>> # Given a document collection "corpus", train word2vec model.
    ...
    def __init__(self, corpus, w2v_model, num_best=None, normalize_w2v_and_replace=True, chunksize=256):
        ...
        w2v_model:       gensim_package         A trained word2vec model.
        ...
        **Do not use this function directly; use the self[query] syntax instead.**
        ...
        if is_corpus:
            ...
            if scipy.sparse.issparse(query):
                ...
                # default case: query is a single vector, in sparse gensim format
                query = matutils.cogensim_packagesc([query], self.index.shape[1], dtype=self.index.dtype)
