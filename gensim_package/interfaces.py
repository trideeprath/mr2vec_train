...
"""
This module contains basic interfaces used throughout the whole gensim_package package.
...
logger = logging.getLogger('gensim_package.interfaces')
...
class CorpusABC(utils.SaveLoad):
    ...
    See the :mod:`gensim_package.corpora.svmlightcorpus` module for an example of a corpus.
...
class TransformationABC(utils.SaveLoad):
    ...
    See the :mod:`gensim_package.models.tfidfmodel` module for an example of a transformation.
...
class SimilarityABC(utils.SaveLoad):
    ...
    def __getitem__(self, query):
        ...
        **or**
        ...
        if self.normalize:
            # self.normalize only works if the input is a plain gensim_package vector/corpus (as
            ...
            if matutils.ismatrix(query):
                import warnings
                # warnings.warn("non-gensim_package input must already come normalized")
            else:
                if is_corpus:
